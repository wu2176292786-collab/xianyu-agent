...
「自动化 → 店铺设置」底部有两个按钮，区别要搞清：

- **清除示例数据**：只删掉示例店铺（商品、消息、订单、流量趋势、示例研究任务），
  同步来的真实数据留着，并把店铺名换成平台上的真名。接了真实账号之后用这个。
- **重置示例数据**：恢复成初始演示状态 —— **真实数据也会一起没了**，得重新同步。

为什么需要「清除」：同步只增不删（一次抓取失败不该把本地数据清空），所以真实商品
进来之后示例店铺还躺在里面，38 件真商品和 11 件「老陈的数码小铺」混在一张表里。
判断哪些是示例数据靠的是**现场生成一份种子状态比对 id**，不是看 id 长得像不像 ——
种子里的 id 是写死的（`L001`、`C001`、`RT001`…），真实 itemId 永远不会撞上。
...
| 同步回来全是已售出 | 带 `needGroupInfo: true` 就能拿到平台自己报的分组件数，直接看出是接口不对还是真的没有在售 |

分组那一条值得单独说：为了搞清「为什么一件在售的都没有」，我们猜了 14 个接口名
（全部「接口不存在」）和 18 个筛选参数（一概被忽略），最后答案在网页版自己的打包
产物里 —— 这个接口支持 `needGroupInfo`，返回「综合 / 在售 / 已售出 / 包邮」
四个分组和各自的件数。同步时会把这句话记进动态：

```
平台分组：综合 38 件，在售 0 件，已售出 38 件，包邮 0 件
```

有了这个数字，「同步回来怎么全是已售出」就不用再靠猜。
